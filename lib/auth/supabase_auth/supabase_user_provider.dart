...
class DeenDiscoverySupabaseUser extends BaseAuthUser {
  DeenDiscoverySupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> deenDiscoverySupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = DeenDiscoverySupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
